...
def test_safe_values_preserve_bool_int_distinction_and_reject_subclasses() -> None:
    ...
    with pytest.raises(UnsafeM7CommandValue): encode_safe_value(Integer(1))
...
def test_durable_reference_reconstruction_is_closed_and_scope_bound() -> None:
    ...
    scope = Scope("t", "w")
    reference = reconstruct_durable_reference({"tenantId":"t","workspaceId":"w","kind":"Memory","identity":"m"}, scope)
    assert reference == DurableReference("t", "w", "Memory", "m")
    with pytest.raises(UnsafeM7CommandValue): reconstruct_durable_reference({"tenantId":"t","workspaceId":"other","kind":"Memory","identity":"m"}, scope)
